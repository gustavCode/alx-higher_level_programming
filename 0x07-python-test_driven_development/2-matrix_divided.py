...
def matrix_divided(matrix, div):
    ...
    for elem in matrix:
        ...
        element_sizes = check_row_size_inconsistency(element_sizes, elem)
        values = []
...
def check_row_size_inconsistency(element_sizes, row):
    """Checks the size consistency of rows in a matrix

    Check if all rows in the matrix are inconsistently sized

    Args:
        element_sizes (:obj:`set` of :obj:`int`): Size of each row matrix.
    ...
    Raises:
        TypeError: If `element_sizes` has more than one size in its contents.

    """

    element_sizes.add(len(row))
